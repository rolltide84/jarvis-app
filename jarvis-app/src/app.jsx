...
function baselineScore(task, context) {
  ...
  const contextFit =
    (task.tags.includes(context.timeOfDay) ? 0.5 : 0) +
    (context.locationTag && task.tags.includes(context.locationTag) ? 0.5 : 0);

  const neglectBoost = task.lastDoneAt
    ? Math.min(2, Math.log1p((now - task.lastDoneAt) / (1000 * 60 * 60 * 24)))
    : 0.3;
...
`;
  ...
  const toggleNotifications = async () => {
    if (!settings.notificationsEnabled) {
      ...
      if (!('Notification' in window)) {
        alert('❌ Notifications are not supported in your browser. Try using Chrome, Firefox, Safari, or Edge.');
        return;
      }
      ...
      try {
        ...
        if (granted) {
          ...
          notificationManager.notify(
            ...
            'You will now receive alerts when tasks are almost due.',
            'test-notification'
          );
        } else {
          alert('⚠️ Notification permission was denied. You can enable it later in your browser settings.');
        }
      } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-white p-6">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={downloadCompletedLog}
                className="p-2 hover:bg-slate-700 rounded-lg transition-colors"
                title="Download completed tasks log"
              >
          ...
          <div className="bg-slate-800 rounded-lg p-6 mb-6 border border-slate-700">
            ...
            <div className="space-y-4">
              ...
              {settings.notificationsEnabled && (
                <div>
                  <label className="block mb-2">
                    Notify me {settings.notificationMinutes} minutes before deadline
                  </label>
                  <input
                    type="range"
                    min="15"
              ...
              <div>
                ...
                <p className="text-sm text-slate-400">
                  Learn from your task choices to improve rankings over time
                </p>
              </div>
              ...
              {settings.enableAdaptive && (
                <div>
                  <label className="block mb-2">
                    Adaptive Influence: {Math.round((1 - settings.adaptiveAlpha) * 100)}%
                  </label>
                  <input
                    type="range"
                    min="0"
                  ...
                  <p className="text-sm text-slate-400 mt-1">
                    Balance between rule-based (left) and learned preferences (right)
                  </p>
                </div>
              )}
              ...
              <div>
                <label className="block mb-2">
                  Available Time (minutes): {context.availableMins}
                </label>
                <input
                  type="range"
                  min="15"
              ...
              <div>
                <label className="flex items-center justify-between">
                  <span>Show Ranking Explanations</span>
                  <input
                    type="checkbox"
        ...
        <button
          ...
          className="w-full bg-blue-600 hover:bg-blue-700 rounded-lg p-4 mb-6 flex items-center justify-center gap-2 transition-colors"
        >
          ...
          {rankedTasks.length === 0 ? (
            ...
            rankedTasks.map((task, index) => (
              <TaskCard
                ...
                task={task}
                rank={index + 1}
                showExplanation={settings.explainRankings}
        ...
        <div className="mt-8 text-center text-sm text-slate-500 space-y-1">
          ...
          <p>No cloud sync • No telemetry • No external calls</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="bg-slate-800 rounded-lg p-6 mb-6 border border-slate-700">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block mb-2 text-sm font-medium">
              Effort: {formData.effortMins} mins
            </label>
            <input
              type="range"
              min="15"
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Set when this task must be completed
          </p>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button
            onClick={onCancel}
            className="flex-1 bg-slate-700 hover:bg-slate-600 rounded-lg py-2 font-medium transition-colors"
          >
            Cancel
          </button>
        </div>
    ...
    <div className={`bg-slate-800 rounded-lg p-4 border transition-colors ${
      ...
      <div className="flex items-start gap-4">
          ...
          <div className="flex items-center gap-4 text-sm text-slate-400 mb-2">
            ...
            {task.dueAt && (
              <span className={isOverdue ? 'text-red-400 font-semibold' : ''}>
                {isOverdue ? '⚠️ OVERDUE' : `Due: ${new Date(task.dueAt).toLocaleString()}`}
              </span>
            )}
          </div>
        ...
        <div className="flex gap-2 flex-shrink-0">
          <button
            onClick={() => onDownloadCalendar(task)}
